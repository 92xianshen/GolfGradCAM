...
def gradcam_fn(input_model, swings, category_index, layer_name):
...
# ->> select swing of id 0, prediction
x, y = x_test[0][np.newaxis], y_test[0]
predictions = model.predict(x)
top_1 = predictions[0].argmax()
predicted_class = top_1
print('Predicted class: {}, Actual class: {}'.format(top_1, y))


for rf in rfs: # for each rf
    # ->> gradient-weighted class activation map (grad-cam)
    cam, heatmap = gradcam_fn(model, x, predicted_class, rf)
    ...
    saliencies = saliency_function(x, guided_model, rf)
    guided_gradcam = saliencies[0].numpy() * heatmap_ext[..., np.newaxis]

    np.savez(
        os.path.join(save_root, '{}.npz'.format(rf)), 
